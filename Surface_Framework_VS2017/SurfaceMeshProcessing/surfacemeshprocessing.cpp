...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actGaosi = new QAction(tr("Gaosi"), this);
	actGaosi->setStatusTip(tr("Show Gaosi"));
	actGaosi->setCheckable(true);
	connect(actGaosi, SIGNAL(triggered()), viewer, SLOT(ShowGAOSI()));
	...
	agViewGroup->addAction(actGaosi);
...
void SurfaceMeshProcessing::CreateMenus(void)
{
	...
	menuRenderMode->addAction(actGaosi);
