...
/**************************************************
@brief   : 计算高斯曲率  参考 https://www.cnblogs.com/VVingerfly/p/4428722.html 中的离散公式
@author  : lee
@input   : none
@output  : none
@time    : none
...
/**************************************************
@brief   : 计算平均曲率   参考    https://computergraphics.stackexchange.com/questions/1718/
...
double MeshViewerWidget::calMeanCurvature(Mesh::VertexIter vertexIndex) {
	OpenMesh::Vec3d P = (mesh).point(*vertexIndex);//中心点坐标
	Point3d vecPoint(P[0], P[1], P[2]);
	std::vector<Point3d> neighborPoints;// N_1 rings points
	for (Mesh::VertexOHalfedgeIter vo_it = mesh.voh_begin(*vertexIndex); vo_it != mesh.voh_end(*vertexIndex); ++vo_it) {//这个顶点所带有的半边迭代器
		OpenMesh::ArrayKernel::VertexHandle to_v = mesh.to_vertex_handle(*vo_it);
		OpenMesh::Vec3d toPoint = mesh.point(to_v);// 邻接点
...
void MeshViewerWidget::DrawGaoSi(void) const {
	glShadeModel(GL_SMOOTH);//均匀插值颜色
	...
	for (const auto& fh : mesh.faces())
	{
		...
		{
			OpenMesh::Vec3uc vcolort = mesh.color(fvh);
			glColor3d(1.0*vcolort[0] / 255, 1.0*vcolort[1] / 255, 1.0*vcolort[2] / 255);
